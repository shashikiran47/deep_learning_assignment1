...
def train_net(data, model, loss_func, optimizer, batch_size, max_epochs,
	...
	- 
    : Decay the learning rate every given epochs
	...
	for epoch in range(max_epochs):
		...
		'''scores = model.forward(data_train)
		predicted_class = np.argmax(scores, axis=1)
		train_acc = np.mean(predicted_class == labels_train)
		
		scores = model.forward(data_val)
		predicted_class = np.argmax(scores, axis=1)
		val_acc = np.mean(predicted_class == labels_val)'''
